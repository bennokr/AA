...
public class ParallelMinimaxQPreyAgent extends ParallelLearningPreyAgent {
    ...
    public void prepare() {
        super.prepare();
        // set the opponent
        mMinimaxQ.setOpponent(getEnvironment().getPredators().get(0));
    }

    @Override
    protected void postActionCallback(State initialState, State resultingState, Action previousAction, Action nextAction) {
    ...
    /**
     * Performs an action during the agent's turn based on the policy, but tripping with a chance of 0.2! Tripping is
     * performing the WAIT action, but storing the chosen action.
     * 
     * @param roundStartState
     *            The state that we act for, or null to act for the environment's current state
     */
    @Override
    public void performAction(final State roundStartState) {
        // Move to a location based on an action determined by the policy
        lastAction = mPolicy.getActionBasedOnProbability(roundStartState != null ? roundStartState : getEnvironment()
                .getState());
